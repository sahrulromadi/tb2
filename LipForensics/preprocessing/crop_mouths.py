...
def crop_video_and_save(video_path, landmarks_dir, target_dir, mean_face_landmarks, args):
    ...
    for i, frame_name in enumerate(frame_names):
        ...
        # Coba muat file landmark. Jika tidak ada, lewati frame ini.
        try:
            landmarks = np.load(landmark_path)
        except FileNotFoundError:
            # Beri pesan peringatan (opsional) dan lanjutkan ke frame berikutnya
            print(f"Peringatan: File landmark tidak ditemukan untuk {frame_name}, dilewati.")
            continue
